...
There were `r total` responses with `r prop(summary_acm, total)`% of respondents being members of the ACM. Membership is higher in North America (`r prop(summary_acm_usa, total_us)`%) and lower in the rest of the world  (`r prop(summary_acm_eu, total_eu)`%). The majority of respondents is from Europe (`r perc(t_orig["Europe"],total)`) with the other biggest chunk being from the USA and Canada (`r perc(t_orig["North America"],total)`). The geographic distribution is shown in the following pie chart.

```{r,echo=FALSE}
pie(summary(data$Origin),main="Origin")
```

The respondent are split between academia (`r perc(t_acad["academia"], total)`) and industry (`r perc(t_acad["industry"],total)`). Lastly, the relative seniority of respondents as measured by the time from their first publication is summarized in the following pie chart.
```{r,echo=FALSE}
...
On the question asking if open access (OA) was a "moral imperative" the vast majority of respondents agreed (`r pDOA(all)`). The other responses where a "a desirable features" and "not essential".  The proportion of respondents who believe that adopting OA is a necessity is different depending on origin and seniority. The following table shows the most significant differences. Respondents outside of the **USA** were  slightly more favorable to OA (`r pDOA(!usa)`). Seniority seemed to play a big role in attitudes, with more junior respondents having stronger feelings (`r pDOA(junior|rest)`). The most striking difference is between **Senior USA Academics** and **Junior USA Academics** the former favoring OA at `r pDOA(academe & senior & usa)` and the latter at a whopping `r pDOA( academe & (junior|rest) & usa )`.
...
The question about how to fund OA divides respondents: `r perc(table(data$Fund_OA)["Author Charges"], total)` are in favor of author processing charges while 
...
More comments about OA include.
...
In the case an author processing charge was selected, how much would be a reasonable price (in USD) for a paper?  The following pie chart breaks the price down.

```{r,echo=FALSE}
pie(table(data$Max_price), main="Reasonable author processing charges ($)")
```

The majority of respondents, `r perc(table(data$Max_price)[1]+table(data$Max_price)[2],total)` to be precise, is willing to pay no more than $100 per paper. An additional `r perc(table(data$Max_price)[3],total)` is willing to pay $200.


The issue of copyrights is fairly clear cut with `r perc(table(data$Copyright)[4],total)` saying that leaving copyrights with the authors is essential.

```{r,echo=FALSE}
pie(summary(data$Copyright),main="Should authors retain copyrights to their work?")
```

The last question was wether it would be reasonable to look for another publisher if an open access option was not available within the ACM by 2017. The marhority of respondents, `r perc(table(data$Switch)[2],total)` was in favor of such a switch.
...
pie(table(data$Switch),main="Publish in OA by 2071")
```

Looking closer reveals some interesting distinctions. The majority ACM members are in favor of looking for an open access publisher by 2017, as with the other results sentiments for OA are stronger in Europe.
