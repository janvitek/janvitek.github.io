...
In fact, `r perc(table(data$Read_journal)[2],total)` of the respondent have not even read a single journal paper in 2014.
...
It is interesting to observe that while 51% of **Junior** authors have published a journal
paper in the last three year, the chances that they will read a journal paper are no signifciantly  different from other age groups. 
...
In comparison, `r perc(table(data$Pub_in_conf)[1],total)` of respondent have published
...
These numbers are remarkable stable accross age groups.
